...
function material_alert ( txt )
{
	if( $('#__al_mod').length == 0){

	dtp = '<div id="__al_mod" class="modal center">' +
		    '<div class="modal-content" id="__al_mod_cnt">' +
		     txt +
		    '</div>' +
			    '<div class="modal-footer">' +
			     ' <a href="#!" class="modal-action waves-effect waves-green btn-flat clx_m">Okay</a>' +
			    '</div>' +
		  	'</div>';
	 ...
	 $('.clx_m').click(function(){
		$('#__al_mod').closeModal();
	 });

	}else{
		$('#__al_mod_cnt').text( txt );
	}
		$('#__al_mod').openModal();
};